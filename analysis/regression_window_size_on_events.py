...
def compute_grid_search_regression_aic(sig_mat_all,story_id_all, button_all, args):
    ...
    y_pred_all=[] #collect predictions for each window size
...
for nfeat in [20, 50, 100, 150, 200]:
    ...
    #corr_score, betas =compute_regression(sig_mat_all,story_id_all, button_all)
    # Compute AIC
    ...
    if compute_aic_per_story:
        suffix= suffix + 'results_by_window_%i' %int(nfeat)
        corr_score, results =compute_grid_search_regression_aic(sig_mat_all,story_id_all, button_all, args)

    np.save(results_dir + '%s_model_predict_%s.npy' % (args.analyses, suffix), results)
